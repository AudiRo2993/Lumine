...
const { EmbedBuilder, PermissionsBitField, Collection, ChannelType } = require("discord.js");
...
module.exports = {
  ...
  async execute(message, client) {
	...
	const command = client.prefix.get(cmd.toLowerCase()) || client.prefix.find(c => c.aliases?.includes(cmd.toLowerCase()));
	...
	if (command.ownerOnly && !staff.includes(message.author.id)) {
		return message.reply({
			embeds: [
				new EmbedBuilder()
					.setColor("Red")
					.setTitle(`<:RZ_WarningIcon:1084203788975472650> ${message.author.username} This command is invalid!`)
			...
			return message.reply({
				embeds: [
					new EmbedBuilder()
						.setColor("Red")
						.setTitle(`<:RZ_WarningIcon:1084203788975472650> ${message.author.username} This command is invalid!`)
						.setDescription(`The command \`${cmd}\` can only be used in our official servers.`)
						.setFooter({ text: `Razen`})
					]
				}).then(m => setTimeout(() => m.delete(), 6000));
			}
