...
const Maintenance = require("../../Structures/models/maintenance");
  module.exports = {
      subCommand: "maintenance.intellisenset",
...
/**
    ...
    async execute(interaction, Razen) {
        ...
        if(!maintenanceData[0].intellisense) {
            ...
            await interaction.reply({ content: "Intellisense Maintenance is now enabled.", ephemeral: true });
        } else {
            await Maintenance.updateOne({}, { $set: { intellisense: false } });
            await interaction.reply({ content: "Intellisense Maintenance is now disabled.", ephemeral: true });
        }
